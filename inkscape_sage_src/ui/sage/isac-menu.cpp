...
{ // called in static void isac_notebook_new (SPWidget *spw)
	...
	*****************************/
	...
	gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(tf), GTK_WRAP_WORD);
	GtkTextBuffer *desc_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(tf));
	gtk_text_buffer_set_text(desc_buffer, "Numerical values shall associate with standard web color text or color code. The last valid condition is the one to have effect. Add colors separated by a pipe symbol to specify stroke/fill: 'red|blue'. "
		"To get transparent sequence, use 'none'. e.g. 'green|none'.", -1);
	gtk_container_add(GTK_CONTAINER(int_expander), tf);
	gtk_widget_show(tf);
...
GtkWidget *isac_menu_opacity_init(GtkObject *spw)
{
	...
	*****************************/
	...
	GtkTextBuffer *desc_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(tf));
	//gtk_text_view_set_editable(GTK_TEXT_VIEW(tf), FALSE);
	gtk_text_buffer_set_text(desc_buffer, "Opacity (inverse of transparency) animation according to max & min. Use digital value for show & hide effect.", -1);
	gtk_container_add(GTK_CONTAINER(int_expander), tf);
	//gtk_table_attach_defaults (GTK_TABLE (tb1), tf, 1, 3, 2, 3);
	gtk_widget_show(tf);
...
GtkWidget *isac_menu_open_init(GtkObject *spw)
{
	...
	*****************************/
	...
	GtkTextBuffer *desc_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(tf));
	//gtk_text_view_set_editable(GTK_TEXT_VIEW(tf), FALSE);
	gtk_text_buffer_set_text(desc_buffer, "Can be used to open new display, preview or to create a graphical plot (for rectangle objects).", -1);
	gtk_container_add(GTK_CONTAINER(int_expander), tf);
	//gtk_table_attach_defaults (GTK_TABLE (tb1), tf, 1, 3, 2, 3);
	gtk_widget_show(tf);
...
GtkWidget *isac_menu_popup_init(GtkObject *spw)
{
	...
	*****************************/
	...
	gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(tf), GTK_WRAP_WORD);
	GtkTextBuffer *desc_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(tf));
	gtk_text_buffer_set_text(desc_buffer, "Can be used to setup a preview URL or trend plot.", -1);
	gtk_container_add(GTK_CONTAINER(int_expander), tf);
	gtk_widget_show(tf);
...
GtkWidget *isac_menu_script_init(GtkObject *spw)
{
	...
	*****************************/
	...
	gtk_text_buffer_set_text(desc_buffer, "Run user's JavaScript when this object is triggered with the selected event or define Vega specification to the object.", -1);
...
GtkWidget *isac_menu_slider_init(GtkObject *spw)
{
	...
	*****************************/
	...
	gtk_text_view_set_wrap_mode(GTK_TEXT_VIEW(tf), GTK_WRAP_WORD);
	GtkTextBuffer *desc_buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(tf));
	gtk_text_buffer_set_text(desc_buffer, "Position graphic between two locations. After entering tag, max & min, press ALT+D to clone object, then drag the clone object to desired max destination. This clone object shall disappear during run-time leaving only the original object positioned at origin which will move along origin & destination proportionally according to max & min value.\n\nTurn on 'Read only' check box to disable dragging.", -1);
	gtk_container_add(GTK_CONTAINER(int_expander), tf);
	gtk_widget_show(tf);
